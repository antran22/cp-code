...
#   ifdef POINTS_EXIT_CODE
#       undef POINTS_EXIT_CODE
#   endif
...
#ifdef THEMIS
    if (result == _ok)
      quit(_fail, "OK verdict should not be used for judging with themis");
#endif // THEMIS
...
#endif // THEMIS
    std::string juryDir,contestantDir;
    std::getline(std::cin, juryDir);
    std::getline(std::cin, contestantDir);
    registerTestlib(3, (juryDir + inputFile).c_str(), (contestantDir + outputFile).c_str(), (juryDir + outputFile).c_str());
}
